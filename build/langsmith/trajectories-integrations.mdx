...
For most LangSmith integrations, both are set for you. The one case that needs manual instrumentation is the [OpenAI Responses API in chaining mode](#openai-responses-api-with-chaining): server-side chaining leaves the wrapper with no natural thread key.
...
- [Claude Agent SDK (Python)](/langsmith/trace-claude-code)
- [Claude Agent SDK (JS)](/langsmith/trace-claude-code)
...
- [Messages view integrations](/langsmith/messages-view-integrations): how the Messages view renders each supported integration.
