...
This notebook shows how to use the basic retrieval functionality, when utilizing Vectara just as a Vector Store (without summarization), including: `similarity_search` and `similarity_search_with_score` as well as using the LangChain `as_retriever` functionality.
...
For more details about Vectara as VectorStore, see the [Vectara docs](https://docs.vectara.com/docs/api-reference/search-apis/lexical-matching).
