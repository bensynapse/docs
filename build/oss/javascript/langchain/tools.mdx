...
A tool with `return_direct=True` can also return a [`Command`](https://reference.langchain.com/javascript/langchain-langgraph/index/Command) to update graph state before the agent exits. When targeting the current graph (`graph` is not set or is `None`), you **must** include a `ToolMessage` in `Command.update`: one entry matching the tool call's `tool_call_id`. Omitting it raises a `ValueError` because every `AIMessage` tool call must have a corresponding `ToolMessage` in the message history.
